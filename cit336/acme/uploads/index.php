...
switch ($action) {
  case 'upload':
    ...
    if ($imageCheck) {
      $message = '<p class="warning">An image by that name already exist.</p>';
    } elseif (empty($invId) || empty($imgName)) {
      $message = '<p class="warning">You must select a product and image file for the product.</p>';
    } else {
      ...
      if ($result) {
        $message = '<p class="success">The upload succeed.</p>';
      } else {
  ...
  case 'delete':
    ...
    //Set a messge based on the delete result
    if ($remove) {
